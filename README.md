...
* [ ] **Model (도메인):** 비즈니스 로직 및 데이터
    * [ ] `Car`: 이름과 위치를 가진 불변 데이터 클래스 구현
    * [ ] `MoveStrategy`: 자동차의 전진 여부를 결정하는 전략 인터페이스 구현
    * [ ] `RacingGameValidator`: 사용자 입력(차 이름, 시도 횟수)의 유효성을 검증하는 객체 구현
    * [ ] `RacingGame`: 자동차 목록을 소유하고, 1회 경주를 실행하며 우승자를 찾는 객체 구현
* [ ] **View (입출력):** 콘솔 입출력 전담
    * [ ] `InputView`: 사용자로부터 자동차 이름, 시도 횟수 입력을 받는 객체 구현
    * [ ] `OutputView`: 콘솔에 라운드별 실행 결과와 최종 우승자를 출력하는 객체 구현
* [ ] **ViewModel (중재자):** View와 Model 사이의 상태 관리 및 로직 컨트롤
    * [ ] `RacingGameViewModel`: View의 요청을 받아 Model을 호출하고, 전체 게임의 흐름을 제어하는 클래스 구현
* [ ] **Application (조립):**
    * [ ] `main` 함수: View, ViewModel, Model(및 전략)을 조립하고 실행 흐름 제어
* [ ] **Test:**
