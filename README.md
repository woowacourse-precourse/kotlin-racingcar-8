...
1. 프로젝트 기본 설정
   - .gitignore 설정과 기본 패키지 구조 생성
   - 애플리케이션 진입점(main) 초기 구성
...
## 커밋/개발 가이드
- 구현 전 이 README의 기능 목록 중 하나를 선택해 커밋 단위로 개발한다.
- 커밋 메시지 형식은 AngularJS Git Commit Message Conventions(유형(scope): 메시지) 스타일을 참고한다. 예: `feat(parser): add car name parser`
